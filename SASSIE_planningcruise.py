...
## Force parameter (True or False):
##  - True: force the redownload of data (if the file has been modified on the server) and creation of maps even if they already exist locally
##  - False: the script won't redownload the data or create the maps if they already exist
##############################################
...
#Force the redownload of data and creation of maps even if they already exist locally
force=True
...
if os.path.isfile(str(rawdata_path)+'/seaice_amsr/'+filename)==False or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/seaice_amsr/'+' https://n5eil01u.ecs.nsidc.org/AMSA/AU_SI12.001/'+str(year)+'.'+str(month).zfill(2)+'.'+str(day).zfill(2)+'/'+filename)   


#MASIE
os.system('mkdir -p '+str(rawdata_path)+'/iceextent_masie')
if os.path.isfile(str(rawdata_path)+'/iceextent_masie/masie_lat_lon_4km.nc')==False or force==True:
...
if os.path.isfile(str(rawdata_path)+'/iceextent_masie/'+filename)==False or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/iceextent_masie/'+' https://masie_web.apps.nsidc.org/pub/DATASETS/NOAA/G02186/netcdf/4km/'+str(year)+'/'+filename)   


#OISST L4
os.system('mkdir -p '+str(rawdata_path)+'/sst_oi')
filename='oisst-avhrr-v02r01.'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.nc'
if os.path.isfile(str(rawdata_path)+'/sst_oi/'+filename)==False or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/sst_oi/'+' https://www.ncei.noaa.gov/data/sea-surface-temperature-optimum-interpolation/v2.1/access/avhrr/'+str(year)+str(month).zfill(2)+'/'+filename)   
...
if os.path.isfile(str(rawdata_path)+'/sss_smapjpl/'+filename)==False or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/sss_smapjpl/'+' https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-protected/SMAP_JPL_L3_SSS_CAP_8DAY-RUNNINGMEAN_V5/'+str(year)+'/'+str(int(doy)-4)+'/'+filename)
...
if os.path.isfile(str(rawdata_path)+'/sss_smos/'+filename)==False or force==True:
...
if len(glob.glob(str(rawdata_path)+'/wind_ccmp/'+filename_dt))<1 or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/wind_ccmp/'+' https://data.remss.com/ccmp/v02.0/Y'+str(year)+'/M'+str(month).zfill(2)+'/'+filename_dt)   
    if (len(glob.glob(str(rawdata_path)+'/wind_ccmp/'+filename_dt))<1 and len(glob.glob(str(rawdata_path)+'/wind_ccmp/'+filename_nrt))<1) or (len(glob.glob(str(rawdata_path)+'/wind_ccmp/'+filename_dt))<1 and force==True):
        os.system('wget -N -P '+str(rawdata_path)+'/wind_ccmp/'+' https://data.remss.com/ccmp/v02.1.NRT/Y'+str(year)+'/M'+str(month).zfill(2)+'/'+filename_nrt)   
...
if len(glob.glob(str(rawdata_path)+'/sla_aviso/'+filename_dt))<1 or force==True:
    os.system('wget -N -P '+str(rawdata_path)+'/sla_aviso/'+' ftp://my.cmems-du.eu/Core/SEALEVEL_GLO_PHY_L4_MY_008_047/cmems_obs-sl_glo_phy-ssh_my_allsat-l4-duacs-0.25deg_P1D/'+str(year)+'/'+str(month).zfill(2)+'/'+filename_dt)   
    if (len(glob.glob(str(rawdata_path)+'/sla_aviso/'+filename_dt))<1 and len(glob.glob(str(rawdata_path)+'/sla_aviso/'+filename_nrt))<1) or (len(glob.glob(str(rawdata_path)+'/sla_aviso/'+filename_dt))<1 and force==True):
        os.system('wget -N -P '+str(rawdata_path)+'/sla_aviso/'+' ftp://nrt.cmems-du.eu/Core/SEALEVEL_GLO_PHY_L4_NRT_OBSERVATIONS_008_046/dataset-duacs-nrt-global-merged-allsat-phy-l4/'+str(year)+'/'+str(month).zfill(2)+'/'+filename_nrt)   
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_iceextent_masie_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.png')==False or force==True:
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_sst_oi_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.png')==False or force==True:
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_sss_smapjpl_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.png')==False or force==True:
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_sss_smos_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.png')==False or force==True:
            ds = xr.open_dataset(filename)
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_wind_ccmp_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'_0.png')==False or force==True:
            ds = xr.open_dataset(filename)
...
if os.path.isfile(filename):
    if os.path.isfile(str(figures_path)+'/mapbeaufort_sla_aviso_'+str(year)+str(month).zfill(2)+str(day).zfill(2)+'.png')==False or force==True:
